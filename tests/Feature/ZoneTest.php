...
it('no longer lets PUT change a zone status', function () {
    ...
    $this->putJson("/api/v1/zones/{$zone->id}", ['name' => 'New', 'status' => 'disabled'])
        ->assertOk()
...
it('refuses a channel manager updating a zone', function () {
    ...
    $this->putJson("/api/v1/zones/{$zone->id}", ['name' => 'Hijacked'])
        ->assertForbidden();
...
it('has no route that deletes a zone', function () {
    ...
    $this->deleteJson("/api/v1/zones/{$zone->id}")->assertStatus(405);
...
it('disables a zone and returns the three impact counts computed live', function () {
    ...
    $this->patchJson("/api/v1/zones/{$zone->id}/status", [
        'status' => 'disabled',
...
it('speaks disabled and stores inactive, in both directions', function () {
    ...
    $this->getJson("/api/v1/zones/{$zone->id}")
        ->assertOk()
        ->assertJsonPath('data.status', 'disabled');
});
...
it('refuses a zone status change with no reason', function () {
    ...
    $this->patchJson("/api/v1/zones/{$zone->id}/status", ['status' => 'disabled'])
        ->assertStatus(422)
...
it('pins the zone status route to 034 so it cannot drift to the 043 family', function () {
    ...
    // And the live route matches the catalog on method and permission. The path prefix
    // differs until the contract migration moves it under /platform/refs — a separate
    // task — so this pins the shape that is settled, not the one that is not.
    $route = collect(Route::getRoutes())->first(
        fn ($r) => $r->uri() === 'api/v1/zones/{zone}/status'
    );

    expect($route)->not->toBeNull()
        ->and($route->methods())->toContain('PATCH')
        ->and($route->gatherMiddleware())->toContain('can:ad.refs.disable');
});
