...
الحالة الحيّة في [`../status/01-platform-admin.md`](../status/01-platform-admin.md) (مولَّدة). هذا الملف هو ترتيب العمل المتبقي: **97 مساراً ناقصاً + 7 على بادئة منحرفة**، مقسّمة إلى تذاكر بترتيب التنفيذ. كل تذكرة: الوحدة التي تملكها، الجداول، الصلاحيات، ما تعتمد عليه، ومعيار القبول. العقد لكل مسار (`b` / `r` / `e`) في `docs/api/catalog/` ولا يُعاد هنا.
...
| PA-02 | الباقات: CRUD كامل | 4 | Tenancy | — | ✅ منجز 2026-09-19 |
| PA-03 | بطاقة القناة: المستخدمون، المستودعات، التغطية | 4 | Tenancy + Identity + Reference | PA-01 | ⬜ |
| PA-04 | طلبات انضمام القنوات | 2 | Tenancy | PA-01 | ⬜ |
| PA-05 | إعادة تعيين مدير القناة | 1 | Identity | PA-01 | ⬜ |
| PA-06 | الاشتراكات وتعيين الباقة والتغيير الجماعي | 4 | PlatformBilling + Tenancy | PA-02 | ⬜ |
| PA-07 | فواتير المنصة والإعفاء والمذكرة الدائنة والتحصيل المتعثر والإيراد | 5 | PlatformBilling | PA-06 | ⬜ |
| PA-08 | مفاتيح الميزات وتجاوزها وميزات القناة | 5 | Tenancy | — | ⬜ |
...
| PA-16 | لوحة القيادة والتقارير والتصدير | 8 | Reporting | PA-06 | ⬜ |
| PA-17 | تصدير القنوات (فردي وجماعي) | 2 | Reporting | PA-16 | ⬜ |
| PA-18 | حذف قناة بموافقة مزدوجة | 1 | Tenancy + Access | PA-01 | ⬜ (المسار حيّ بصلاحية `role:platform_admin`؛ الموافقة المزدوجة والشروط ناقصة) |
...
- **قبول:** إنشاء باقة ثم قراءتها تُعيد الشكل في `r`؛ تعديل الحدود ينعكس على `GET /platform/channels/{id}/usage` لقناة على الباقة دون تغيير في الشيفرة (معيار BE4-BIL01 الأول)؛ `key` فريد → 422؛ الأسعار أعداد صحيحة (القاعدة 7).
