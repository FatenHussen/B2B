...
| [`plan/`](plan/) | **What is left**, as ordered tickets. `platform-admin.md` / `apps.md` / `channel-catalog.md` (launch tickets — done). Remaining work: [`backend-finish.md`](plan/backend-finish.md) (ops + debt + permissions + finish definition) and [`channel-warehouse-v1.1.md`](plan/channel-warehouse-v1.1.md) (post-launch depth). Each ticket names module, dependencies and acceptance. | Edited by hand when a ticket lands (flip its status) or the catalog changes. |
