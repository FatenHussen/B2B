...
| [`plan/`](plan/) | **What is left**, as ordered tickets. `platform-admin.md` for the back office, `apps.md` for the mobile apps and the shared surface, `channel-catalog.md` for catalog deepen tickets. Each ticket names module, tables, permissions, dependencies and acceptance. | Edited by hand when a ticket lands (flip its status) or the catalog changes. |
