...
use Modules\Finance\Application\Actions\DecideCreditApproval;
...
use Modules\Finance\Application\Queries\ListCreditApprovals;
...
use Modules\Finance\Presentation\Http\Requests\DecideCreditApprovalRequest;
...
final class ChannelFinanceController extends ApiController
{
    ...
    public function creditApprovals(ListCreditApprovals $query, Request $request): JsonResponse
    {
        return $this->paginated($query($request), fn ($row) => $query->map($row));
    }

    public function decideCreditApproval(DecideCreditApprovalRequest $request, DecideCreditApproval $action, int $id): JsonResponse
    {
        return $this->ok($action($request->user(), $id, $request->validated()));
    }
}
